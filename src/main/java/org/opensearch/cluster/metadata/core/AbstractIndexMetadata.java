...
import static org.opensearch.cluster.metadata.MetadataUtils.CONTEXT_MODE_API;
import static org.opensearch.cluster.metadata.MetadataUtils.CONTEXT_MODE_GATEWAY;
import static org.opensearch.cluster.metadata.MetadataUtils.CONTEXT_MODE_PARAM;
...
/**
 * Base class for index metadata information.
 *
 * @opensearch.api
 */
@PublicApi(since = "3.2.0")
public class AbstractIndexMetadata<T> implements Diffable<T>, ToXContentFragment, VerifiableWriteable {
    ...
    public static Setting<Integer> buildNumberOfShardsSetting() {
    ...
    public static final String DEFAULT_NUMBER_OF_SHARDS = "opensearch.index.default_number_of_shards";
    public static final String MAX_NUMBER_OF_SHARDS = "opensearch.index.max_number_of_shards";
     ...
     * with their primary.  Search replicas require the use of Segment Replication on the index and poll their {SegmentReplicationSource} for
     ...
     */
    public static final Setting<ShardCount> SETTING_WAIT_FOR_ACTIVE_SHARDS = new Setting<>(
        "index.write.wait_for_active_shards",
        "1",
        ShardCount::parseString,
    ...
    public static final Setting<String> INGESTION_SOURCE_POINTER_INIT_RESET_SETTING = Setting.simpleString(
        ...
        new Setting.Validator<>() {
            ...
            private boolean isValidResetState(String value) {
                return ResetState.LATEST.name().equalsIgnoreCase(value)
                    || ResetState.EARLIEST.name().equalsIgnoreCase(value)
                    || isResetState(value);
            }

            private boolean isResetState(String value) {
                return ResetState.RESET_BY_OFFSET.name().equalsIgnoreCase(value)
                    || ResetState.RESET_BY_TIMESTAMP.name().equalsIgnoreCase(value);
            }
    ...
    static final String REMOTE_SNAPSHOT_KEY = "remote_snapshot";

    public static final String INDEX_STATE_FILE_PREFIX = "state-";

    protected final int routingNumShards;
    protected final int routingFactor;
    protected final int routingPartitionSize;

    protected final int numberOfShards;
    protected final int numberOfReplicas;
    protected final int numberOfSearchOnlyReplicas;
    ...
    protected final long settingsVersion;

    protected final long aliasesVersion;

    protected final long[] primaryTerms;

    protected final State state;

    protected final Map<String, AliasMetadata> aliases;

    protected final Settings settings;

    protected final Map<String, AbstractMappingMetadata> mappings;

    protected final Map<String, DiffableStringMap> customData;

    protected final Map<Integer, Set<String>> inSyncAllocationIds;

    protected final transient int totalNumberOfShards;

    protected final DiscoveryNodeFilters requireFilters;
    protected final DiscoveryNodeFilters includeFilters;
    protected final DiscoveryNodeFilters excludeFilters;
    protected final DiscoveryNodeFilters initialRecoveryFilters;

    protected final Version indexCreatedVersion;
    protected final Version indexUpgradedVersion;

    protected final ShardCount waitForActiveShards;
    protected final Map<String, RolloverInfo> rolloverInfos;
    protected final boolean isSystem;
    protected final boolean isRemoteSnapshot;

    protected final int indexTotalShardsPerNodeLimit;
    protected final int indexTotalPrimaryShardsPerNodeLimit;
    protected final boolean isAppendOnlyIndex;

    protected final Context context;
    protected final IngestionStatus ingestionStatus;

    protected AbstractIndexMetadata(
        final Index index,
        final long version,
        ...
        this.isRemoteSnapshot = REMOTE_SNAPSHOT_KEY.equals(INDEX_STORE_TYPE_SETTING.get(this.settings));
     ...
     * that can be indexed into) is larger than 0. See org.opensearch.cluster.routing.allocation.IndexMetadataUpdater#applyChanges.
     **/
    ...
    public IngestionSource getIngestionSource() {
        ...
        if (ingestionSourceType != null && !(NONE_INGESTION_SOURCE_TYPE.equals(ingestionSourceType))) {
            final ResetState pointerInitResetType = ResetState.valueOf(
     ...
     */
    public ShardCount getWaitForActiveShards() {
    ...
    public Map<String, AbstractMappingMetadata> getMappings() {
     ...
     */
    @Nullable
    public AbstractMappingMetadata indexMapping() {
        for (final AbstractMappingMetadata cursor : mappings.values()) {
            return cursor;
        }
        return null;
    }
    ...
    public Map<String, DiffableStringMap> getCustomData() {
    ...
    public boolean equals(Object o) {
        ...
        AbstractIndexMetadata<?> that = (AbstractIndexMetadata<?>) o;
    ...
    public Diff<T> diff(T previousState) {
        return (Diff<T>) new AbstractIndexMetadataDiff((AbstractIndexMetadata<?>) previousState, this);
    }

    public static Diff<AbstractIndexMetadata<?>> readDiff(StreamInput in) throws IOException {
        return new AbstractIndexMetadataDiff(in);
    }

    public static AbstractIndexMetadata<?> fromXContent(XContentParser parser) throws IOException {
     ...
     * @opensearch.internal
     */
    protected static class AbstractIndexMetadataDiff implements Diff<AbstractIndexMetadata<?>> {
        ...
        private final Diff<Map<String, AbstractMappingMetadata>> mappings;
        ...
        public AbstractIndexMetadataDiff(AbstractIndexMetadata<?> before, AbstractIndexMetadata<?> after) {
        ...
        private static final DiffableUtils.DiffableValueReader<String, AbstractMappingMetadata> MAPPING_DIFF_VALUE_READER =
            new DiffableUtils.DiffableValueReader<>(AbstractMappingMetadata::new, AbstractMappingMetadata::readDiff);
        ...
        public AbstractIndexMetadataDiff(StreamInput in) throws IOException {
            index = in.readString();
        ...
        @Override
        public AbstractIndexMetadata<?> apply(AbstractIndexMetadata<?> part) {
    ...
    public static AbstractIndexMetadata<?> readFrom(StreamInput in) throws IOException {
        ...
        for (int i = 0; i < mappingsSize; i++) {
            AbstractMappingMetadata mappingMd = new AbstractMappingMetadata(in);
    ...
    public static Builder builder(AbstractIndexMetadata<?> indexMetadata) {
    ...
    public static class Builder {
        ...
        protected long mappingVersion = 1;
        protected long settingsVersion = 1;
        protected long aliasesVersion = 1;
        protected long[] primaryTerms = null;
        protected Settings settings = Settings.Builder.EMPTY_SETTINGS;
        protected final Map<String, AbstractMappingMetadata> mappings;
        protected final Map<String, AliasMetadata> aliases;
        protected final Map<String, DiffableStringMap> customMetadata;
        protected final Map<Integer, Set<String>> inSyncAllocationIds;
        protected final Map<String, RolloverInfo> rolloverInfos;
        protected Integer routingNumShards;
        protected boolean isSystem;
        protected Context context;
        protected IngestionStatus ingestionStatus;
        ...
        public Builder(AbstractIndexMetadata<?> indexMetadata) {
         ...
         * See {@link AbstractIndexMetadata#primaryTerm(int)} for more information.
         */
        public long primaryTerm(int shardId) {
        ...
        protected void initializePrimaryTerms() {
        ...
        public AbstractIndexMetadata<?> build() {
            ...
            Version indexUpgradedVersion = settings.getAsVersion(AbstractIndexMetadata.SETTING_VERSION_UPGRADED, indexCreatedVersion);
            ...
            final ShardCount waitForActiveShards = SETTING_WAIT_FOR_ACTIVE_SHARDS.get(settings);
            ...
            final int indexTotalPrimaryShardsPerNodeLimit = INDEX_TOTAL_PRIMARY_SHARDS_PER_NODE_SETTING.get(settings);
            ...
            return new AbstractIndexMetadata<>(
        ...
        public static void toXContent(AbstractIndexMetadata<?> indexMetadata, XContentBuilder builder, Params params) throws IOException {
            ...
            builder.field(KEY_STATE, indexMetadata.getIndexState().toString().toLowerCase(Locale.ENGLISH));
            ...
            if (context != XContentContext.API) {
                ...
                AbstractMappingMetadata mmd = indexMetadata.indexMapping();
                if (mmd != null) {
                    if (binary) {
            ...
            if (context != XContentContext.API) {
                ...
                builder.startObject(AbstractIndexMetadata.KEY_PRIMARY_TERMS);
                        ...
                        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                            if (token == XContentParser.Token.FIELD_NAME) {
                                ...
                                Map<String, Object> mappingSource = MapBuilder.<String, Object>newMapBuilder()
                                    ...
                                    .map();
                                builder.putMapping(new AbstractMappingMetadata(mappingType, mappingSource));
                            } else {
                    ...
                    if (KEY_MAPPINGS.equals(currentFieldName)) {
                        while ((token = parser.nextToken()) != XContentParser.Token.END_ARRAY) {
                            if (token == XContentParser.Token.VALUE_EMBEDDED_OBJECT) {
                                builder.putMapping(new AbstractMappingMetadata(new CompressedXContent(parser.binaryValue())));
                            } else {
                                Map<String, Object> mapping = parser.mapOrdered();
                                if (mapping.size() == 1) {
                                    String mappingType = mapping.keySet().iterator().next();
                                    builder.putMapping(new AbstractMappingMetadata(mappingType, mapping));
                                }
                            }
                        }
    ...
    protected static final ToXContent.Params FORMAT_PARAMS;
    static {
     ...
     * <p>
     * This looks for the presence of the {@link Version} object with key {@link AbstractIndexMetadata#SETTING_VERSION_CREATED}
     */
    public static Version indexCreated(final Settings indexSettings) {
        ...
        if (indexVersion.equals(Version.V_EMPTY)) {
            final String message = String.format(
                ...
                indexSettings.get(AbstractIndexMetadata.SETTING_INDEX_UUID)
            );
     ...
     * {org.opensearch.cluster.routing.OperationRouting#generateShardId(IndexMetadata, String, String)} to route documents
     ...
     */
    public static ShardId selectSplitShard(int shardId, AbstractIndexMetadata<?> sourceIndexMetadata, int numTargetShards) {
     ...
     */
    public static ShardId selectCloneShard(int shardId, AbstractIndexMetadata<?> sourceIndexMetadata, int numTargetShards) {
        int numSourceShards = sourceIndexMetadata.getNumberOfShards();
    ...
    private static void assertSplitMetadata(int numSourceShards, int numTargetShards, AbstractIndexMetadata<?> sourceIndexMetadata) {
        if (numSourceShards > numTargetShards) {
            throw new IllegalArgumentException(
     ...
     */
    public static Set<ShardId> selectRecoverFromShards(int shardId, AbstractIndexMetadata<?> sourceIndexMetadata, int numTargetShards) {
     ...
     */
    public static Set<ShardId> selectShrinkShards(int shardId, AbstractIndexMetadata<?> sourceIndexMetadata, int numTargetShards) {
        if (shardId >= numTargetShards) {
            throw new IllegalArgumentException(
                "the number of target shards (" + numTargetShards + ") must be greater than the shard id: " + shardId
            );
     ...
     * {org.opensearch.cluster.routing.OperationRouting#generateShardId(IndexMetadata, String, String)} to guarantee consistent
